    ...
    var utils = UM.utils,
        ...
        getAllPic: function (sel, $w, editor) {
            ...
            $.each($imgs, function (index, node) {
            	...
            	if (UM.browser.ie && UM.browser.version <= 10) {
                    var img = new Image();
                    img.src = node.src;
                    me.scale(node, 350, img.width, img.height);
                }else {
                	me.scale(node, 350, node.naturalWidth, node.naturalHeight);
                }
        ...
        scale: function (img, max, oWidth, oHeight) {
            var width = 0, height = 0, percent, ow, oh;
            if (UM.browser.ie && UM.browser.version <= 10) {
                var imgwh = new Image();
                imgwh.src = img.src;
                ow = oWidth || imgwh.width, oh = oHeight || imgwh.height;
            }else {
                ow = oWidth || img.width, oh = oHeight || img.height;
            }
            if (ow > max || oh > max) {
                if (ow >= oh) {
                    if (width = ow - max) {
                        percent = (width / ow).toFixed(2);
                        $(img).attr("height",(oh - oh * percent).toFixed(2));
                        $(img).attr("width",max);
                        $(img).css({"height":(oh - oh * percent).toFixed(2),"width":max});
                    ...
                    if (height = oh - max) {
                        ...
                        $(img).attr("width",(ow - ow * percent).toFixed(2));
                        $(img).attr("height",max);
                        $(img).css({"width":(ow - ow * percent).toFixed(2),"height":max});
        ...
        submit: function (callback) {
            ...
            $(me.dialog).delegate( ".edui-image-file", "change", function ( e ) {
                ...
                $('<iframe name="up"  style="display: none"></iframe>').insertBefore(me.dialog).on('load', function(){
                    var r = this.contentWindow.document.body.innerText || this.contentWindow.document.body.textContent;
                    if(r == '')return;
                    me.uploadComplete(r);
                    $(this).unbind('load');
                    $(this).remove();
